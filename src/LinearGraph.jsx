...
return ( <div className="flex items-center justify-center min-h-screen bg-white font-[Calibri] py-24 px-20"> <div className="w-full max-w-7xl grid gap-20 grid-cols-1 lg:grid-cols-2">

{[1, 2].map((chartIndex) => (
      <motion.div
        key={chartIndex}
        className={`p-10 rounded-3xl bg-white shadow-xl border border-gray-200 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 ${chartIndex === 1 ? 'h-[720px]' : 'h-[720px]'}`}
      ...
      >
        <div className="flex justify-between items-start mb-3">
          <p className="text-xs text-gray-700 text-left">Numéro d'incident : {chartIndex === 1 ? 'INC0123478' : 'INC0987654'}</p>
          <div className="flex flex-col items-end">
            <h2 className="text-sm font-extrabold text-gray-950 mb-1 tracking-tight">Évolution du traitement de l'incident</h2>
            <p className="text-xs text-gray-700 mb-1">{chartIndex === 1 ? "Demande de restauration d'archive à la date du 25 Octobre 2025" : "Analyse secondaire — 25 Octobre 2025"}</p>
        ...
        <div className="w-full h-[600px] bg-gradient-to-b from-[#e3f6f5] to-[#ffffff] rounded-xl px-10 py-12 flex items-end justify-center relative overflow-hidden">
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 40, right: 60, left: 60, bottom: 80 }}>
              ...
              }} activeDot={{ r: 6 }} fill={`url(#colorValue${chartIndex})`} isAnimationActive animationDuration={1500} style={{ filter: 'drop-shadow(0px 4px 6px rgba(39,35,67,0.6))' }} />
            </LineChart>
          </ResponsiveContainer>
