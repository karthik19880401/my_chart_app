import React, { useState } from 'react'; import { LineChart, Line, XAxis, ResponsiveContainer, ReferenceArea, ReferenceLine } from 'recharts'; import { motion } from 'framer-motion'; import { AiOutlineLine } from 'react-icons/ai';

const data = [ { name: '9:00 AM', hour: 9, value: 60, label: "CDS - Niv 1" }, { name: '10:00 AM', hour: 10, value: 60, label: "CDS - Niv 2" }, { name: '1:00 PM', hour: 13, value: 60, label: "SAU - HL 2" }, { name: '2:00 PM', hour: 14, value: 60, label: "SAU - Data" }, { name: '4:10 PM', hour: 16.17, value: 60, label: "CDS - Niv 3" }, { name: '5:00 PM', hour: 17, value: 60, label: "CDS - Niv 1" } ];
...
return ( <div className="min-h-screen bg-white font-[Calibri] py-32 px-20 relative"> {/* Fixed Header */} <header className="fixed top-0 left-0 w-full bg-white shadow-md border-b border-gray-200 z-50 p-6"> <h1 className="text-xl font-bold text-gray-900 mb-2"> Demande de restauration d'une archive Outlook. </h1> <p className="text-sm text-gray-700 leading-relaxed max-w-5xl"> Ci-dessous, vous avez une vue d'ensemble de la procédure pour traiter une demande de restauration d'une archive, chacune des étapes est à appliquer consciencieusement, elle est décrite sur le panel à droite. La ligne verticale du graph vous permet de voir les différents groupes d'affectation et leur temps d'intervention, le graph sinusoidal vous montre le temps de traitement, la différence entre le temps de traitement d'une phase prétablie et le temps pris pour le faire. </p> </header>

{/* Main Content */}
  <div className="w-full max-w-7xl mx-auto grid gap-12 grid-cols-1 lg:grid-cols-2 pt-12">
    {[1, 2].map((chartIndex) => (
      <motion.div
        key={chartIndex}
      ...
      >
        ...
        {chartIndex === 1 && (
          <div className="mt-6 flex justify-end items-center">
            <span className="text-xs text-gray-700 mr-2">Résolu par</span>
            <select
              value={resolver}
