...
import { AiOutlineLine, AiOutlineCloud, AiOutlineClockCircle, AiOutlineBarChart, AiOutlineSetting, AiOutlineCheckCircle } from 'react-icons/ai';
...
export default function LinearGraph() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-white font-[Calibri] py-20 px-20">
      <div className="w-full max-w-7xl grid gap-12 grid-cols-[6%,1fr,1fr]">

        {/* Left Icon Column */}
        <motion.div
          className="flex flex-col justify-around items-center bg-white rounded-3xl shadow-md py-8"
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.7 }}
        >
          {[AiOutlineCloud, AiOutlineClockCircle, AiOutlineBarChart, AiOutlineSetting, AiOutlineCheckCircle].map((Icon, index) => (
            <Icon key={index} className="text-[#272343] text-2xl opacity-80 hover:opacity-100 transition-all duration-300" />
          ))}
        </motion.div>

        {/* Intro Section */}
        <motion.div
          className="col-span-2 p-8 bg-white shadow-xl rounded-3xl"
        ...
        >
          ...
          <p className="text-sm text-gray-700 leading-relaxed">
            ...
            Chacune des étapes est à appliquer consciencieusement et est décrite sur le panneau à droite. 
            La ligne verticale du graphique vous permet de visualiser les différents groupes d'affectation et leur temps d'intervention, 
            tandis que le graphique sinusoïdal illustre le temps de traitement — la différence entre le temps de traitement préétabli et 
            le temps réellement pris pour le faire.
          </p>
        </motion.div>
        ...
        >
          <p className="text-xs text-gray-700 text-left mb-4">Numéro d'incident : INC0123478</p>

          <div className="w-full h-[300px] bg-gradient-to-b from-[#e3f6f5] to-[#ffffff] rounded-xl px-10 py-10 flex items-end justify-center relative overflow-hidden">
            <ResponsiveContainer>
                  ...
                  dot={(props) => {
                    ...
                    return (
                      <g>
                        ...
                        {label && (
                          <text x={cx + 10} y={cy - 14} textAnchor="start" fontSize={10} fill="#272343" fontFamily="Calibri">
                            {label}
                          </text>
                        )}
                      </g>
