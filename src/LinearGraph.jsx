  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white font-[Calibri] py-20 px-20">
      ...
      <div className="w-full max-w-7xl grid gap-12 grid-cols-1 lg:grid-cols-3">
        {/* --- New Left Component --- */}
        <motion.div
          className="p-10 rounded-3xl bg-white shadow-xl border border-gray-200 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 h-[370px]"
        ...
        >
          ...
          <p className="text-xs text-gray-600 leading-relaxed mt-8">
            Cet incident représente la première phase du processus. Il contient la collecte initiale des
            données nécessaires avant la restauration complète de l'archive. Les étapes de validation
            administratives précèdent l'affectation technique.
          </p>
        </motion.div>
        ...
        {[1, 2].map((chartIndex) => (
          <motion.div
            ...
            className={`p-10 rounded-3xl bg-white shadow-xl border border-gray-200 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 ${
              chartIndex === 1
                ? "h-[370px]"
                : "h-[370px] w-[90%] justify-self-center"
            }`}
            initial={{ opacity: 0, y: 40 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
