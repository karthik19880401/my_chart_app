...
import { AiOutlineLine, AiOutlineMail, AiOutlineClockCircle, AiOutlineUser, AiOutlineCheckCircle, AiOutlineDatabase } from "react-icons/ai";
...
export default function LinearGraph() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white font-[Calibri] py-20 px-20">
      ...
      <motion.div
        className="max-w-6xl mb-10 bg-white text-left p-8 rounded-3xl transition-all duration-500"
      ...
      >
        ...
        <p className="text-sm text-gray-700 leading-relaxed max-w-4xl">
          Ci-dessous, vous avez une vue d'ensemble de la procédure pour traiter une demande de restauration
          d'une archive, chacune des étapes est à appliquer consciencieusement, elle est décrite sur le panel à
          droite. La ligne verticale du graph vous permet de voir les différents groupes d'affectation et leur
          temps d'intervention, le graph sinusoïdal vous montre le temps de traitement, la différence entre le
          temps de traitement d'une phase prétablie et le temps pris pour le faire.
        </p>
      </motion.div>

      {/* --- Charts Section --- */}
      <div className="w-full max-w-7xl grid gap-12 grid-cols-1 lg:grid-cols-3">
        {/* --- Left Component with Icons --- */}
        <motion.div
          className="p-10 rounded-3xl bg-white shadow-xl border border-gray-200 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 h-[370px] flex flex-col justify-center items-center gap-8"
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: 'easeOut' }}
          whileHover={{ scale: 1.01 }}
        >
          <AiOutlineMail className="text-4xl text-[#272343] opacity-90" />
          <AiOutlineClockCircle className="text-4xl text-[#272343] opacity-90" />
          <AiOutlineDatabase className="text-4xl text-[#272343] opacity-90" />
          <AiOutlineUser className="text-4xl text-[#272343] opacity-90" />
          <AiOutlineCheckCircle className="text-4xl text-[#272343] opacity-90" />
        </motion.div>
